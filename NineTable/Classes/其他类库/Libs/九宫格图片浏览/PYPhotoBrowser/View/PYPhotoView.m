...
// cell的高
#define PYPhotoCellH (_photoCell.height > 0 ? _photoCell.height : PYScreenH)
...
- (instancetype)init
{
    if (self = [super initWithFrame:CGRectZero]) {
        ...
        // 设置图片为默认图片
        self.image = [UIImage imageWithColor:HWRandomColor];
    }
    return self;
}
...
- (void)setAnimatedImage:(FLAnimatedImage *)animatedImage
{
    ...
    if (height > PYPhotoCellH) { // 长图
        if (self.isBig) { // 预览状态
            self.size = CGSizeMake(PYPhotoCellW, PYPhotoCellW * animatedImage.size.height / animatedImage.size.width);
        }
    }
...
- (void)photoDidRotation:(UIRotationGestureRecognizer *)rotation
{
    ...
    if (rotation.state == UIGestureRecognizerStateEnded
        ...
        [UIView animateWithDuration:0.18 delay:0 options:UIViewAnimationOptionCurveEaseIn animations:^{
            ...
            contentScrollView.size = CGSizeMake(self.width > PYPhotoCellW ? PYPhotoCellW : self.width, self.height > PYPhotoCellH ? PYPhotoCellH : self.height);
            contentScrollView.contentSize = self.size;
            contentScrollView.center = CGPointMake(PYPhotoCellW * 0.5, PYPhotoCellH * 0.5);
            contentScrollView.contentOffset = CGPointZero;
...
- (void)imageDidPinch:(UIPinchGestureRecognizer *)pinch
{
    ...
    if (pinch.state == UIGestureRecognizerStateChanged) {
        ...
        if (self.width > PYPhotoCellW * PYPreviewPhotoMaxScale && pinch.scale > 1.0) {
...
- (void)setPhoto:(PYPhoto *)photo
{
    ...
    UIImage *placeholdeerImage = [UIImage imageWithColor:HWRandomColor];
    if (self.isBig) { // 图片浏览（放大）, 原图具有优先级
        // 获取缩略图
        placeholdeerImage = self.photo.thumbnailImage ? self.photo.thumbnailImage : [UIImage imageWithColor:HWRandomColor];
    ...
    [self sd_setImageWithURL:url placeholderImage:placeholdeerImage options:0 progress:^(NSInteger receivedSize, NSInteger expectedSize) {
        ...
        if (image && ([[imageURL absoluteString] isEqualToString:self.photo.thumbnail_pic] ||
            ...
            [self addGestureRecognizers];
            // 记录原始大小
            self.photo.originalSize = CGSizeMake(self.width, self.width * image.size.height / image.size.width);
            // 记录未旋转的宽度或者旋转完成时的宽度
...
- (void)collectionViewDidScroll:(NSNotification *)noti
{
    ...
    if (!CGAffineTransformEqualToTransform(self.transform, CGAffineTransformIdentity) && ((self.photoCell.x >= scrollView.contentOffset.x + scrollView.width) || (CGRectGetMaxX(self.photoCell.frame) < scrollView.contentOffset.x)) && (self.width >= PYPhotoCellW || self.photoCell.contentScrollView.transform.a)) { //self.transform不为初始化状态并且不在屏幕上并且有缩放或者旋转，就要初始化
        ...
        contentScrollView.y = (PYRealyScreenH - contentScrollView.height) * 0.5;
