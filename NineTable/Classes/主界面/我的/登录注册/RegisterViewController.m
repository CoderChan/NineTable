...
- (void)setupSubViews
{
    ...
    [protocolButton setTitle:@"我已阅读并接受《九宫格用户协议》" forState:UIControlStateNormal];
    protocolButton.titleLabel.font = [UIFont preferredFontForTextStyle:UIFontTextStyleCaption2];
    protocolButton.frame = CGRectMake(20, CGRectGetMaxY(self.passField.frame)+15, 220, 30);
    [protocolButton setTitleColor:[UIColor grayColor] forState:UIControlStateNormal];
    [protocolButton setImage:[UIImage imageNamed:@"selected_btn"] forState:UIControlStateNormal];
    [protocolButton setImage:[UIImage imageNamed:@"selected_btn"] forState:UIControlStateHighlighted];
    protocolButton.titleEdgeInsets = UIEdgeInsetsMake(0, 5, 0, 0);
    [protocolButton addBlockForControlEvents:UIControlEventTouchUpInside block:^(id  _Nonnull sender) {
    ...
    registerButton.frame = CGRectMake(40, CGRectGetMaxY(protocolButton.frame)+120, self.view.width - 80, 44);
...
- (void)registerAction
{
    ...
    [[NineTableManager sharedManager].netManager registerWithPhone:self.nameField.text Pass:self.passField.text Code:self.codeField.text Success:^(int code) {
        [MBProgressHUD hideHUD];
        if (code == 1) {
            [self.navigationController popToRootViewControllerAnimated:YES];
        }else if (code == 2){
            [self sendAlertAction:@"参数错误"];
        }else if (code == 3){
            [self showPopTipsWithMessage:@"账号已注册，请登录" AtView:self.nameField inView:self.view];
        }else{
            [self sendAlertAction:@"请核对账号、验证码、密码"];
        }
    } Fail:^(NSString *errorMsg) {
        [MBProgressHUD hideHUD];
